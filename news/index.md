...
- Code Generator: closed the missing parenthesis on the `strat_levels`
  line emitted for stratified cross-tabs. The generated `.R` script now
  parses cleanly (#BUG-03).
...
- [`read_dataset()`](https://ramsas88.github.io/quickexplorer/reference/read_dataset.md):
  preserve `haven` `label` / `format.sas` attributes and the
  `haven_labelled` class when converting blank SAS strings to `NA`
  (#BUG-05).
  [`get_variable_info()`](https://ramsas88.github.io/quickexplorer/reference/get_variable_info.md)
  once again populates `Label` and `Format` for SAS inputs.
- [`read_dataset()`](https://ramsas88.github.io/quickexplorer/reference/read_dataset.md):
  reject `.rds` files whose top-level object is not a `data.frame`, with
  a clear error message instead of cryptic downstream failures
  (#BUG-20).
- [`format_file_size()`](https://ramsas88.github.io/quickexplorer/reference/format_file_size.md):
  extended units to `TB` and `PB` and added a guard for negative sizes
  (#BUG-07).
- [`get_variable_info()`](https://ramsas88.github.io/quickexplorer/reference/get_variable_info.md):
  `N_Unique` now counts distinct non-missing values, matching `skimr`
  and `DataExplorer` (#BUG-08). Missing counts continue to be reported
  separately in `Missing_Count`.
- [`compute_numeric_summary()`](https://ramsas88.github.io/quickexplorer/reference/compute_numeric_summary.md)
  ...
  emit a clear error naming the offending variable when `vars` includes
  a name that is not in `df`, instead of base R’s
  `undefined columns selected` (#BUG-09).
- [`compute_numeric_summary()`](https://ramsas88.github.io/quickexplorer/reference/compute_numeric_summary.md):
  now returns `Q1` and `Q3` quartile columns alongside the existing
  summary statistics (#BUG-13).
- [`compute_crosstab()`](https://ramsas88.github.io/quickexplorer/reference/compute_crosstab.md):
  validate `row_var`, `col_var`, and `strat_var` up-front and fail with
  an actionable error if any is missing or duplicated (#BUG-15).
- [`compute_crosstab()`](https://ramsas88.github.io/quickexplorer/reference/compute_crosstab.md):
  warn when the source data carries values literally equal to `Total` or
  `(Missing)`, which would otherwise be indistinguishable from synthetic
  totals / NA-relabel cells (#BUG-14).
- Converter: the CSV preview now honours the user’s delimiter and
  header-row toggles, matching what the download handler writes
  (#BUG-11).
- Converter: rewrote XPT variable-name shortening so a `make.unique`
  step is no longer followed by truncation, avoiding duplicate
  8-character names; if any name is shortened, a `_name_map.csv` is
  emitted alongside the `.xpt` so analysts can trace renames (#BUG-10).
...
- Documented
  [`compute_crosstab()`](https://ramsas88.github.io/quickexplorer/reference/compute_crosstab.md),
  the Cross-tabulation panel, the Code Generator tab, and the SAS
  blank-to-NA coercion that were already shipped in 0.1.0 (#BUG-18).

### Tests

- Added `tests/testthat/test_quickexplore_suite.R`: testthat edition-3
  suite with eight thematic sections (~40 test cases) covering
  file-format dispatch, metadata, numeric/categorical summaries,
  cross-tabulation, converter round-trip, code-generator parseability,
  and data-type coverage.
