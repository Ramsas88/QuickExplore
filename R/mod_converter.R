...
#'   \describe{
#'     \item{`output_format`}{A [shiny::reactive()] returning the selected output format string.}
#'     \item{`csv_delim`}{A [shiny::reactive()] returning the CSV delimiter character.}
#'     \item{`json_pretty`}{A [shiny::reactive()] returning `TRUE` to pretty-print JSON output.}
#'   }
#'
...
converter_server <- function(id, loaded_data, selected_dataset) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    list(
      output_format = shiny::reactive(input$output_format),
      csv_delim     = shiny::reactive(input$csv_delim),
      json_pretty   = shiny::reactive(input$json_pretty)
    )
  })
}
